...
def fetch_points(account_number, auth_key, proxy):
    headers = {
    ...
    while True:
        try:
            response = requests.get(url, headers=headers, proxies=proxy_dict, verify=False)
...
with ThreadPoolExecutor(max_workers=len(auth_keys)) as executor:
    futures = [executor.submit(fetch_points, i + 1, key, proxies[i % len(proxies)]) for i, key in enumerate(auth_keys)]
